 ...
 package com.mphasis.fileapplication.api; import static org.mockito.Mockito.*;
 ...
 @ExtendWith(MockitoExtension.class) class FileControllerTest {
 private MockMvc mockMvc;
 @Mock private FileService fileService;
 @InjectMocks private FileController fileController;
 @BeforeEach void setUp() { mockMvc =
 MockMvcBuilders.standaloneSetup(fileController).build(); }
 
 @Test
 void testDeleteFile() { doNothing().when(fileService).deleteFile(1L);
 assertDoesNotThrow(() -> fileController.deleteFile(1L));
 verify(fileService, times(1)).deleteFile(1L); 
 }
  @Test void testUpdateStatus() {
 ...
 @Test void testGetFile_Success() { FileEntity fileEntity = new FileEntity();
 fileEntity.setId(1L); fileEntity.setFilename("TestFile.txt");
 ...
 @Test void testGetFile_NotFound() {
 when(fileService.getFile(99L)).thenReturn(null);
 Exception exception = assertThrows(ResourceNotFoundException.class, () ->
 fileController.getFile(99L));
